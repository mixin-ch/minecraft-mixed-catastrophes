...
public class AspectScoreManager {
    ...
    public void updateScoreboard(Player player) {
        ...
        int playerIndex = playerIdList.indexOf(player.getUniqueId());

        if (playerIndex == -1) {
            playerIdList.add(playerId);
            playerIndex = playerIdList.size() - 1;
        }
        ...
        for (AspectType aspectType : aspectOrder) {
            ...
            if (value > 0) {
                makeScore(scoreboard, objective, playerIndex, index, Constants.AspectThemes.get(aspectType).getColor(), aspectType.getLabel(), value);
                index++;
            }
        }

        if (dreamCooldown > 0) {
            makeScore(scoreboard, objective, playerIndex, index, Constants.DreamThemes.get(DreamType.SereneDreams).getColor(), "Dreamless", dreamCooldown, "s");
            index++;
        }

        if (antiLighthouseTimer > 0) {
            makeScore(scoreboard, objective, playerIndex, index, Constants.AspectThemes.get(AspectType.Terror).getColor(), "Red Eye", antiLighthouseTimer, "s");
            index++;
        }
    ...
    private void makeScore(Scoreboard scoreboard, Objective objective, int playerId, int index, ChatColor chatColor, String label, int value) {
        makeScore(scoreboard, objective, playerId, index, chatColor, label, value, "");
    }

    private void makeScore(Scoreboard scoreboard, Objective objective, int playerId, int index, ChatColor chatColor, String label, int value, String valueUnit) {
